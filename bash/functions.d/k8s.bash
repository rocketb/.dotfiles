...
function _custom_k8s_parse_args() {
	declare -a args; args=()
...
kwatch() { kubectl get -w $(_custom_k8s_parse_args "$@") | cut -c-$(tput cols); }
# shellcheck disable=SC2046
kget() { kubectl get $(_custom_k8s_parse_args "$@") | cut -c-$(tput cols); }
# shellcheck disable=SC2046
kdesc() { kubectl describe $(_custom_k8s_parse_args "$@"); }

if [[ $(type -t compopt) = 'builtin' ]]; then
	__custom_complete_k8s_get() {
	...
	complete -o default -F __custom_complete_k8s_get kwatch
	complete -o default -F __custom_complete_k8s_get kget
fi

# Select resource with labels, sorted by ascending age.
function kgsort() {
	kubectl get "$(_custom_k8s_parse_args "$@")" \
...
function _custom_k8s_select_pod() {
	local kind="${1:-pods}"
	shift
...
alias kpdesc='_custom_k8s_select_pod | xargs kubectl describe pod -n'
alias kpdelete='_custom_k8s_select_pod | xargs kubectl delete pod -n'
alias kplog='_custom_k8s_select_pod | xargs kubectl logs -f -n'
alias kpman='_custom_k8s_select_pod | xargs kubectl get pod -o json -n | fx'
alias kpip='_custom_k8s_select_pod | xargs kubectl get pod -o jsonpath="{.status.podIPs[*]}" -n | jq'
alias ksdockerconfig="_custom_k8s_select_pod secrets --field-selector type=kubernetes.io/dockerconfigjson | xargs kubectl get secret -o go-template='{{index .data \".dockerconfigjson\" | base64decode }}' -n | jq"

function ksecretdecode() {
	local args secret_key
	read -ra args < <(_custom_k8s_select_pod secret)
	if [ ${#args} -lt 1 ]; then
		return
	fi
...
function kexec() {
	read -ra tokens < <(_custom_k8s_select_pod)
	if [ ${#tokens} -gt 1 ]; then
...
# DEPLOYMENTS
# ---
alias krestart='_custom_k8s_select_pod deployments | xargs kubectl rollout restart deployment -n'
...
if [[ $(type -t compopt) = 'builtin' ]]; then
	...
	complete -o default -F __custom_complete_k8s_nodes knode
	complete -o default -F __custom_complete_k8s_nodes ktaints
	complete -o default -F __custom_complete_k8s_nodes kdebug
fi
